...
describe('evaluate', () => {
  ...
  it('should report info if the predicate and filter are both present', function () {
    ...
    const evaluationHandler: EvaluationHandler = new FilterShouldExistIfPredicateEvaluationHandler();
    const result: HandlerCheckResult[] = evaluationHandler.handle(pdSchema, vpSimple);
    expect(result[0]).toEqual(new HandlerCheckResult('root.input_descriptors[0].constraints.fields[0]', '', 'FilterShouldExistIfPredicate', Status.INFO, "predicate value and the filter value are both present."));
  });
});